...
% Parameters
numUsers = 10;       % use the 10 users (U1-U10) provide
sampleRate = 30;    
windowSeconds = 2;  
...
% Use the 'First Day' (FD) for train and 'Second Day' (MD) for test
allFeatures_Train = [];
allLabels_Train = [];
allFeatures_Test = [];
allLabels_Test = [];
...
disp('Load data and extract features');

for u = 1:numUsers
    fprintf('Process User %d...\n', u);
    
    % Load Train Data (First Day - FD)
    filename_fd = sprintf('U%dNW_FD.csv', u);
    try
        data_fd = readmatrix(filename_fd);
        
    catch ME
        warning('Error! %s. Check file path and format.', filename_fd);
        disp(ME.message);
        continue;
    end
    ...
    % Load Test Data (Second Day - MD)
    filename_md = sprintf('U%dNW_MD.csv', u);
    try
        data_md = readmatrix(filename_md);
    catch ME
        warning('Error! %s. Check file path and format.', filename_md);
        disp(ME.message);
        continue;
    end
...
% Analyze Features
disp('Analyze feature use t-SNE.');
...
title('t-SNE (t-distributed Stochastic Neighbor Embedding) Plot of Train Features');
xlabel('t-SNE Dimension 1');
ylabel('t-SNE Dimension 2');
legend('User 1', 'User 2', 'User 3', 'User 4', 'User 5','User 6','User 7','User 8','User 9','User 10', 'Location', 'best');
grid on;

disp('Feature analysis plot generate');

% Create the scatter plot
disp('Generate scatter plot');
...
disp('Scatter plot generate');

% Design and Train Neural Network
disp('Train Neural Network Classifier');
